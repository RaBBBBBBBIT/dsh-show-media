...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const FETCH_PATH = "/api/dsh-show-media";
    ...
    function MediaCard(props) {
      ...
      React.useEffect(function () {
        ...
        function fallbackAttachment() {
          ...
          // A card can outlive its live session binding (history, reconnects,
          // and browser-use child sessions). Use the authenticated remote API
          // directly first; it only needs the durable session id and attachment
          // id, not an in-memory Session object.
          const remoteSession = props.remote && props.remote.session;
          if (remoteSession && typeof remoteSession.attachment === "function") {
            remoteSession.attachment({ sessionId: sessionId, attachmentId: attachmentId }).then(function (result) {
              if (cancelled) return;
              if (!result || result.ok !== true || !result.value || typeof result.value.data !== "string") {
                fallbackBoundSession();
                return;
              }
              ...
              const mediaType = result.value.attachment && result.value.attachment.mediaType
                ...
                : "image/png";
              useBlob(bytes, mediaType, "image");
            }).catch(function () {
              if (!cancelled) fallbackBoundSession();
            });
            return;
          }
          fallbackBoundSession();
        ...
        // Images are already durably stored as attachments by the host tool;
        // avoid a session-less host filesystem RPC for them. Videos have no
        // image attachment and continue to use the original-file RPC path.
        if (attachmentId) {
          fallbackAttachment();
          return function () {
            cancelled = true;
        ...
        fetch(FETCH_PATH, {
          ...
          if (!response.ok) throw new Error("media route returned HTTP " + response.status);
      ...
      React.useEffect(function () {
        ...
        if (mediaKind === "video") {
          media.controls = true;
          media.autoplay = true;
          media.style.cssText = "position:relative;max-width:98vw;max-height:96vh;width:auto;height:auto;object-fit:contain;border-radius:0;box-shadow:var(--dsw-shadow-lv3, 0 8px 32px rgba(0,0,0,.35));background:#000;";
        } else {
          media.alt = path || "image";
          media.style.cssText = "position:relative;max-width:98vw;max-height:96vh;width:auto;height:auto;object-fit:contain;border-radius:0;box-shadow:var(--dsw-shadow-lv3, 0 8px 32px rgba(0,0,0,.35));cursor:zoom-out;background:var(--dsw-alias-bg-base);";
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("tool.call.toolview", () => ctx.slots.register(
        ...
        function BoundCard(props) {
          return React.createElement(MediaCard, Object.assign({}, props, { sessions: sessions, remote: remote }));
        },
      ));
    }
